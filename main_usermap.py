...
def summarize_group_history(group_name, raw_history):
    ...
    for m in raw_history[-25:]:
        ...
        # Build Discord ID -> username map
        discord_user_map = {}  # populate from group members dynamically

        if config.DISCORD_ID:
            content = replace_discord_mentions(
                content, discord_user_map, config.DISCORD_ID
            )
...
# ---------------------------
# Helper: replace all Discord mentions
# ---------------------------
def replace_discord_mentions(text, discord_user_map, bot_id):
    """
    Replace <@ID> mentions with @Username.
    Replace bot's own ID with @you.
    """
    if not text:
        return text
    ...
    def repl(match):
        ...
        if uid == str(bot_id):
            return "@you"
        return f"@{discord_user_map.get(uid, 'Unknown')}"
...
def get_roast_response(user_message, group_name, sender_name):
    ...
    if not is_private_env and trimmed_group:
        ...
        for entry in last_20:
            ...
            # Build a Discord ID -> username map for this group
            # For group history, you can maintain this dynamically as well
            discord_user_map = {}  # fill with actual mappings: {id: username}

            if config.DISCORD_ID:
                c = replace_discord_mentions(c, discord_user_map, config.DISCORD_ID)
...
def psi09():
    try:
        ...
        # Build a Discord ID -> username map for this group
        discord_user_map = {str(sender_name): sender_name}  # minimal fallback
        # In practice, populate with actual IDs and usernames from your Discord client
        user_message = replace_discord_mentions(
            user_message, discord_user_map, config.DISCORD_ID
        )
